...
/*
class TranslatorPy : public Translator {
private:
	LLVMContext ctx;
	API api;
public:
	TranslatorPy() : Translator(ctx,api) {}
};

PYBIND11_MODULE(transTritonLLVM, m) {

	py::class_<TranslatorPy>(m, "Translator")
		.def(py::init<>())
		.def("TritonAstToLLVMIR",&TranslatorPy::TritonAstToLLVMIR)
		.def("LLVMIRToTritonAst",&TranslatorPy::LLVMIRToTritonAst);
	//m.def("free_translator",&free_translator,"free");
}
*/
#endif
